...
public class EmployeeActivity extends AppCompatActivity {
    ...
    // Despite my best attempts, I was unable to find a way to make the delete function without
    // Dropping the ENTIRE table
    public void deleteEmployeeFromDB(String last){
        SQLiteDatabase database = new SampleDBSQLiteHelper(this).getWritableDatabase();
        database.delete(SampleDBContract.Employee.TABLE_NAME, "COLUMN_LASTNAME=?", new String[] {last.toString()});
    }

    public void deleteFromDB(View v) {
        TextView last;

        last = (TextView)  findViewById(R.id.lastnameLabel);

        String lastN = last.getText().toString();
        deleteEmployeeFromDB(lastN);
    }
}